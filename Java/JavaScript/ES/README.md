...
- 循环语句：通过循环语句可以反复的执行一段代码多次
  ...
  - break关键字可以用来退出switch或循环语句
    ...
    - 可以为循环语句创建一个label，来标识当前的循环。label:循环语句，使用break语句时，可以在break后跟着一个label，这样break将会结束指定的循环，而不是最近的
...
JS中数据类型：String 字符串；Number 数值；Boolean 布尔值；Null 空值；Undefined 未定义
以上这五种类型属于基本数据类型，以后我们看到的值，只要不是上边的5种，全都是对象：**Object 对象**
